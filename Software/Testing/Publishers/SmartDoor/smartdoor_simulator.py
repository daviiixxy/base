...
def simulate_smartdoor(ambiente):
    ...
    try:
        ...
        print(f"Não foi possível conectar ao broker em {BROKER_IP}:{BROKER_PORT}. Erro: {e}")
        return
    ...
    print(f"Estados iniciais publicados: Porta='{last_door_state}', Luz='{last_light_state}'")
    
    # Agenda a primeira mudança
    next_door_update_time = time.time() + random.randint(DOOR_INTERVAL_MIN, DOOR_INTERVAL_MAX)
    next_light_update_time = time.time() + random.randint(LIGHT_INTERVAL_MIN, LIGHT_INTERVAL_MAX)
    ...
    try:
        while True:
            ...
            if current_time >= next_door_update_time:
                ...
                next_door_update_time = time.time() + random.randint(DOOR_INTERVAL_MIN, DOOR_INTERVAL_MAX)
                print(f"Próxima verificação da porta em {int(next_door_update_time - current_time)} segundos.")
            ...
            if current_time >= next_light_update_time:
                ...
                next_light_update_time = time.time() + random.randint(LIGHT_INTERVAL_MIN, LIGHT_INTERVAL_MAX)
                print(f"Próxima verificação da luz em {int(next_light_update_time - current_time)} segundos.")
            
            time.sleep(1) # Loop principal roda a cada segundo para não sobrecarregar a CPU
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simulador de dispositivo SmartDoor com intervalos realistas.")
    parser.add_argument(
        "--ambiente",
        type=str,
        required=True,
